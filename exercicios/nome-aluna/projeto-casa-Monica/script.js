...
button.addEventListener('click', (Event) => {
    ...
    const bicho = document.getElementById('bicho').value
    getdigimon(bicho)
})

function criarDigimonPesquisado(bicho) {
    return `
      <div class="bicho">
        <img src=${bicho.img} class="image">
        
      </div>
    `
  }

async function getdigimon(bicho)  {
try {
   
    const response = await fetch(`https://digimon-api.vercel.app/api/digimon/name/${bicho}`)
    const json = await response.json()
    const divDigimon = document.getElementById("digimon")
    divDigimon.appendChild(criarDigimonPesquisado(json[0]))
  }
...
function fillFields(digimon){

   document.getElementById('bicho').value = digimon.bicho
}

async function main(bicho) {
    if(!isNaN(bicho)){
  const digimon = await getdigimon(bicho)
  fillFields(bicho)
} else{
